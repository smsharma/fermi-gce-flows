...
\noindent
\textbf{Prior specification:} Priors for the Poissonian templates are specified uniformly on the corresponding template normalizations. For the PS component, priors on the broken power-law parameters ($\theta_\mathrm{PS}$ in Eq.~\eqref{eq:scd_bpl}) characterizing the distribution of photon counts are motivated as follows. The break associated with the brighter end of the SCD, $S_{\mathrm{b}, 1}  \in  [5, 40]$\,photons, reflects a `turn-on' associated with a source luminosity function, above which sources are either individually resolved or do not exist. This turn-on is further enforced by specifying a steeply falling highest slop $n_1 \in [10, 20]$. 

PS emission is partially degenerate but still statistically distinguishable from a Poisson distribution when associated with $\sim 0.1--5$ counts in expectation~\cite{List:2021}; in practice, however, residual effects of model misspecification and degeneracies between multiple PS populations can make characterizing the source-count distribution in this low-photon regime challenging within our framework. The dimmer break, $S_{\mathrm{b}, 2} \in  [0.1, 4.99]$\,photons, is therefore associated with the sensitivity threshold of the analysis. We allow for a steeply rising lowest slope $n_3 \in [-10, 20]$, encouraging the SCD to turn off in this regime. The middle slope $n_2 \in [1.1, 1.99]$ is associated with the physical luminosity function of the source population. 



For the PS components, rather than sampling the expected per-pixel number of PSs $\lambda$ with a uniform prior, we instead uniformly sample a related parameter $\langle S^{\rm PS} \rangle = \int\,\dd S\,S\,\lambda \, p(S\mid\theta_\mathrm{PS})$, the expected number of photon counts contributed by the PS population per pixel. 
