...
batch_size_list = [128]
fc_dims_list = [[[-1, 2048], [2048, 128]], [[-1, 2048], [2048, 512]]]
maf_num_transforms_list = [4]
maf_hidden_features_list = [128]
methods = ["snpe"]
activations = ["relu"]
kernel_size_list = [8]
conv_channel_configs = ["standard", "more_channels"]
...
# aux_summaries = ["None"]
# n_aux_list = [2]
# # aux_summaries = ["None", '["pca_96"]', '["pspec_4"]', '["pspec_4","pca_96"]']
# # n_aux_list = [2, 2 + 96, 2 + 95, 2 + 96 + 95]
...
#                                     for aux_summary, n_aux in zip(aux_summaries, n_aux_list):
#                                         batchn = batch + "\n"
#                                         batchn += "python -u train.py --sample train_ModelO_gamma_fix_480k --name gce_ModelO_gamma_fix_480k_geometric --method {} --maf_num_transforms {} --maf_hidden_features {} --fc_dims '{}' --batch_size {} --activation {} --kernel_size {} --laplacian_type {} --chebconv {} --aux_summary {} --n_aux {}".format(method, maf_num_transforms, maf_hidden_features, fc_dims, batch_size, activation, kernel_size, laplacian_type, chebconv, aux_summary, n_aux)
#                                         fname = "batch/submit.batch"
#                                         f = open(fname, "w")
#                                         f.write(batchn)
#                                         f.close()
